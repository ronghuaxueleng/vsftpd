...
```bash
docker pull ronghuaxueleng/vsftpd
```
...
```bash
  docker run --rm ronghuaxueleng/vsftpd
```

2) Create a container in active mode using the default user account, with a binded data directory:

```bash
docker run -d -p 21:21 -v /my/data/directory:/home/vsftpd --name vsftpd ronghuaxueleng/vsftpd
...
--name vsftpd --restart=always ronghuaxueleng/vsftpd
```
